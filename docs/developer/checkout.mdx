...
### Why there is no cart model?

Unlike most of other ecommerce platforms, Saleor has no separate model for a cart. Instead, we decited to merge it with checkout. Main reason was providing additional address context, required to calculate taxes in different countries.
Checkout provides interface for usual cart operations like managing adding products or promo codes. 
...
mutation {
  checkoutLinesAdd(
    checkoutId: "Q2hlY2tvdXQ6MzQwNTFhMDAtNTI3NC00OWEwLWE3YmEtMTE0NjUwMWY3MzM3"
...
Response contains updated checkout contents:
```json
{
  "data": {
    "checkoutLinesAdd": {
...
If variant is already added, its quantity will be increased. Running prevoius mutation twice will result in checkout with one line and quantity set to 2:
```json
{
  "data": {
    "checkoutLinesAdd": {
